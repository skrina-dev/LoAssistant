...
struct GetCharacterDataService {
    ...
    func getMarketInfo(URL: String, completion : @escaping (NetworkResult<[Pricechart]>) -> Void) {
    ...
    private func judgeStatus(by statusCode: Int, _ data: Data) -> NetworkResult<[Pricechart]> {
    ...
    private func isValidData(data: Data) -> NetworkResult<[Pricechart]> {
        ...
        guard let decodedData = try? decoder.decode(MarketModel.self, from: data) else { return .pathErr }
        
        // 성공적으로 decode를 마치면 success에다가 data 부분을 담아서 completion을 호출합니다.
        return .success(decodedData.characterList!)
    }
}
